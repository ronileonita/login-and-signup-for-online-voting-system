...
def Login():
	...
	def App(uid,user):
		...
		btn=Button(win,text="View the sorted items",command=sorteditem,bg='#ffffff',font=(10))
		...
		btn=Button(win,text="View ",command=view, bg='#ffffff',font=(10))
		...
		btn=Button(win,text="View your graph",command=ViewGraph,bg='#ffffff',font=(10))
	...
	userInp = Entry()
	userInp.place(x=render.width()-30,y=render.height()-250, height=30)

	psw = Label(img,text="Password" , bg="white", font=f1)
	psw.place(x=(render.width()/2)-135,y=render.height()-200)
	pswInp = Entry()
	pswInp.place(x=render.width()-30,y=render.height()-200, height=30)
